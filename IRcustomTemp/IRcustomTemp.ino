...
//ID Device
String idDevice = "b59a613c-6628-4a07-b4ee-27956834d09f";
//String humidifier = "4b76b78d-30d9-4a46-8e63-1a8341b2348d";
//String ac = "93cd88bc-4a07-11ed-b878-0242ac120002";
//String listDevice = "[\"" + ac + "\",\"" + humidifier + "\"]";
...
uint16_t ONLGac[16] = { -21000, 8850, -4050, 500, -1600, 200, -100, 100, -650, 150, -850, 150, -2900, 150, -950, 100 };
uint16_t OFFLGac[18] = { -22958, 8850, -4050, 650, -1500, 150, -900, 100, -3950, 150, -950, 100, -4150, 100, -1750, 150, -850, 200 };
// Example Samsung A/C state captured from IRrecvDumpV2.ino
uint8_t samsungState[kSamsungAcStateLength] = {
  0x02, 0x92, 0x0F, 0x00, 0x00, 0x00, 0xF0,
  0x01, 0xE2, 0xFE, 0x71, 0x40, 0x11, 0xF0
};
int valueChangetemp,ac_temperature;
...
void stream() {
  ...
  if (httpResponseCode != 200) {
    ...
    ac_temperature = obj["result"]["suhu"]["number"].as<int>();
    valueChangetemp = obj["result"]["suhu"]["value"].as<int>();
    if (obj["result"]["on"].as<int>() == 1) {
      Serial.println("AC nyala");
      // irsend.sendRaw(ONac, 349, 38);  // Send a raw data capture at 38kHz.
      // irsend.sendRaw(ONLGac, 16, 38);
      // Ac_Activate(ac_temperature, ac_flow, ac_heat);
      ac.on();
      ac.send();
    } else if (obj["result"]["off"].as<int>() == 1) {
      Serial.println("AC mati");
      // irsend.sendRaw(OFFac, 349, 38);  // Send a raw data capture at 38kHz.
      // irsend.sendRaw(OFFLGac, 18, 38);
      // Ac_Power_Down();
      ac.off();
      ac.send();
    }
    if (valueChangetemp == 1) {
      Serial.println(ac_temperature);
      // Ac_Activate(ac_temperature, ac_flow, ac_heat);
      ac.on();
      ac.setMode(kSamsungAcCool);
      ac.setTemp(ac_temperature);
      ac.send();
    } else {
    }
    // if (obj["result"][humidifier]["on"].as<int>() == 1) {
    //   Serial.println("HUM nyala");
    //   //irsend.sendRaw(ONhum, 71, 38);  // Send a raw data capture at 38kHz.
    // } else if (obj["result"][humidifier]["off"].as<int>() == 1) {
    //   Serial.println("HUM mati");
